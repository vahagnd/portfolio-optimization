...
test_prices, test_returns = stock_data.preprocess(years=YEARS, prices=PRICES, normalize=None, scaler=SCALER)[4:]
...
def test_all(
    ...
    logger.info(f"testing_data.shape: {testing_data.shape}, testing_returns.shape: {testing_returns.shape}")
    model.eval()
    with torch.no_grad():
    ...
    plt.close()
    logger.info("Saved test cumulative return plot.")


if __name__ == "__main__":
    logger.info("test.py executed directly")
    fix_seed(SEED)
    test_all(
        test_prices=test_prices,
        test_returns=test_returns,
