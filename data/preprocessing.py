...
class Data:
    ...
    def __prepreprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        new_columns = [
            (feat.split(".")[0], ticker) for feat, ticker in df_clean.columns
        ]
    ...
    def __preprocess(
        ...
        train_returns = (
            (train_prices - train_prices.shift(1)) / train_prices.shift(1)
        ).iloc[1:]
        val_returns = ((val_prices - val_prices.shift(1)) / val_prices.shift(1)).iloc[
            1:
        ]
        test_returns = (
            (test_prices - test_prices.shift(1)) / test_prices.shift(1)
        ).iloc[1:]
    ...
    def get_train_val_datasets(
        self, window_size: int = 32, sharpe_window: int = 25, **kwargs
    ) -> tuple[StockDataset, StockDataset]:
        """
        Get training and validation datasets based on the preprocessed data.
        """
        train_p, train_r, val_p, val_r, _, _ = self.__preprocess(**kwargs)
        train_ds = StockDataset(
            train_r, train_p, window_size=window_size, sharpe_window=sharpe_window
        )
        val_ds = StockDataset(
            val_r, val_p, window_size=window_size, sharpe_window=sharpe_window
        )
    ...
    def get_test_dataframes(self, **kwargs) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        Get test prices and returns based on the preprocessed data.
        """
        _, _, _, _, test_prices, test_returns = self.__preprocess(**kwargs)
